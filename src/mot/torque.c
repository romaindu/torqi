...
{
    g_torque.inhibit_encoder = 1;
    g_torque.trq = 0;
    g_torque.phi[PHASE_A] = ECNT_PER_MAGREV/4;
    g_torque.phi[PHASE_B] = ECNT_PER_MAGREV/2;
    g_torque.trq = 100;

    for (uint32_t i = 0; i < 0x6fffff; ++i) __asm__("nop");

    g_torque.inhibit_encoder = 0;
    g_torque.trq = 0;
    g_torque.phi[PHASE_A] = 0;
    g_torque.phi[PHASE_B] = ECNT_PER_MAGREV/4;
}
